...
public class HidingAI : MonoBehaviour
{
    ...
    [SerializeField] private AudioClip enemyDeadSound; //audio clip
    [SerializeField] private AudioClip alertSound; //audio clip
    ...
    void Update()
    {
        if (_alive)
        {
            ...
            if (GameObject.Find("Player") && GameObject.Find("Player").GetComponent<PlayerCharacter>()._health >= 0)
            {
                ...
                if (sqrLen < closeDistance * closeDistance)
                {
                    ...
                    //Plays alert sound when enemy shoots
                    if (!soundSource.isPlaying && isAlerted)
                    {
                        soundSource.PlayOneShot(alertSound);
                        isAlerted = false;
        ...
        else
        {
            ...
            if (!soundSource.isPlaying){
                soundSource.PlayOneShot(enemyDeadSound);
            }
        }
    }
